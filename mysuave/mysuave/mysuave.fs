...
let getJenkins domain jobNames: WebPart =
    let fetchBuildResult path number = async {
        let! buildResult =  BuildResultContext.AsyncLoad(sprintf "http://%s/%s/%d/api/json?pretty=true" domain path number)
...
let app =
    ...
    choose [
        GET >=> choose [
            pathScan "/api/runs/%s/%s" (fun (domain, jobNames) -> getJenkins domain (splitPath jobNames))
        ]
    ]
